/**
 * Repeatable real-PTY rendering load probe for PRD §4.1 and C-PERF-05.
...
async function renderSession(id: number, mibPerSession: number) {
  const marker = `ELWOOD_RENDER_COMPLETE_${id}`;
  const final = `\n${marker}\n`;
  const expectedBytes = mibPerSession * 1024 ** 2 + Buffer.byteLength(final) + 2;
  const writer = `
  ...
  const pty: PtyProcess = {
    ...
    onData: (handler) =>
      raw.onData((data) => {
        ...
        maxQueuedBytes = Math.max(maxQueuedBytes, queued);
        handler(data);
      }),
    flowControl: {
      pause: () => {
  ...
  const observeFrame = frameObserver(String(id));
  ...
  try {
    ...
    const childExitToRenderMs = Math.round(performance.now() - exitAt);
    if (
      exitCode !== 0 ||
      signal ||
      rendered !== expectedBytes ||
      !terminal.snapshot().text.includes(marker)
    ) {
      throw new Error(
        `Session ${id}: exit=${exitCode}, signal=${signal}, rendered=${rendered}/${expectedBytes}`,
      );
    }
