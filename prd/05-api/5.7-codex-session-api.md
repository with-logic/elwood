...
continues after a listener failure. Each failed delivery emits one live
`transcript_listener_error` warning with only its bounded `channel` label and fixed
message/raw text; it never includes the thrown value or transcript content. Warning
