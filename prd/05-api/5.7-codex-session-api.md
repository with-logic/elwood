...
Codex initial-ready status/activity Promise rejections outside a hook report one
content-free `initial_ready_observer_failed` warning with
`raw: "initial_ready_observer_failed phase=lifecycle"`. Readiness triggered within
`SessionStart` shares that hook's observer boundary and warning limit. Neither
diagnostic delays the ready transition or queue release.
