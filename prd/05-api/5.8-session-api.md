...
the ergonomic `send`/`stream` turns and scheduled loop delivery: before its initial
submission, an ergonomic turn holds due loop input until its real boundary,
including acceptance recovery and post-failure transcript drain. A loop already
queued behind that hold cannot prevent the caller's submission or recovery.
