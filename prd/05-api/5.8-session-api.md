...
`turnId` (Codex), the facade binds collection to the first non-empty string `turn_id`
on the matching `UserPromptSubmit` hook, or to
the first tagged content event if none arrived earlier. A `Stop` hook cannot establish
a binding because it may be delayed from an earlier turn. Later differently tagged
content is ignored; untagged content remains collectable. Missing, empty, or non-string
hook IDs do not establish a binding. Claude transcript activity carries no `turnId`, so for Claude the
