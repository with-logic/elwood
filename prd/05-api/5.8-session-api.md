...
jump ahead of a successor caller whose serialized slot is already reserved. If a
loop’s queue operation has already started, the caller waits for that loop’s completion and
trailing transcript drain before attaching its own turn collector or submitting.
When several loop operations start in succession, each waits for the previous
loop's trailing drain before submitting; a caller waits for every earlier loop.
