...
the ergonomic `send`/`stream` turns and scheduled loop delivery: once its slot is
reserved and the underlying session is available, an ergonomic turn holds due loop
input until its real boundary,
including acceptance recovery and post-failure transcript drain. A loop already
queued behind that hold cannot prevent the caller's submission or recovery, or
jump ahead of a successor caller whose serialized slot is already reserved.
The raw turn-producing control methods
(`sendMessage`, `sendPrompt`, `sendGuidance`) bypass the ergonomic turn queue,
but still honor readiness and selected loop reservations. A raw submission waits
for earlier selected loops to finish physical submission, completion, and trailing
transcript drain; non-turn controls such as model selection can pass that wait.
Calling a raw method CONCURRENTLY with an in-flight `send`/`stream`
