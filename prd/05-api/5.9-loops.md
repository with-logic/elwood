...
loop cancellation discards pending delivery under the existing rules.

A selected loop reserves physical turn order through its completion and trailing
transcript drain, including after its input write settles. Later raw messages,
prompts, guidance, and successor loops wait behind that reservation without
occupying the physical control slot; independent non-turn controls can pass.
Cancelling a waiting operation removes only its reservation, and closing rejects
pending raw input. Cancelling an already committed loop does not release its
still-running turn or trailing drain.
