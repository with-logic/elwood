...
loop cancellation discards pending delivery under the existing rules. A loop whose queue operation has already started keeps its own completion
boundary: later ergonomic callers wait for its physical submission to settle and
its completion and trailing drain, so its hooks and content cannot
be attributed to the caller. Closing the session cancels that wait. A failed or cancelled loop submission
discards its observer and releases waiting callers under existing readiness rules.
