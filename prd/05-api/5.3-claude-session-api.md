...
short settle delay. Bounded recovery may retry only from a completed native
frame newer than the frame observed before the preceding Enter, with the draft
still staged and input idle and unblocked. An unchanged cached draft does not
authorize another Enter: if a swallowed Enter produces no fresh qualifying
render, recovery stops without guessing. Native `user_message` evidence or
observed native work permanently revokes that submission's recovery, even when
it arrives before the physical Enter promise settles. Optimistic caller
submission status alone does not count as native acceptance. Old native activity
may conservatively revoke recovery; this does not establish turn correlation.
The ordinary submission promise still resolves after its first Enter dispatch,
and cancellation cleanup retains its existing ownership contract.
