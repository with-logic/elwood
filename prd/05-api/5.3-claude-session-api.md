...
quiet within one second, or a render failed and the screen has not been fully
redrawn since — the write is withheld
...
follow the same rule. `sendKeys` remains the explicit raw-input escape hatch and
does not wait.
...
`terminal.snapshot()`. The read-only boolean property `terminal.renderFailed` is
true from a failed render until a later write erases and redraws the whole screen
(an incremental write cannot restore the lost output), so a snapshot taken while
it is true may be stale. Visual terminal renderers SHOULD render the raw
