...
completion signals such as an unblocked `Stop` hook. A Stop captures the current
queue-backed caller submission before notifying observers or invoking handlers. If a newer
`sendPrompt`, `sendMessage`, or `sendGuidance` prompt is physically submitted while
that Stop is being handled, the older Stop
MUST NOT mark the newer submission ready or release its queued successor.
Immediate input remains available inside Stop callbacks, including a handler
that awaits `sendPrompt()`; waiting for Stop completion must not deadlock it.
Blocked outcomes still retain the active turn, and failed-open callbacks use the
same submission boundary as an ordinary unblocked outcome. Queued messages are
