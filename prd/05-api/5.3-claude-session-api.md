...
The same rule binds Elwood's own AUTOMATED startup-prompt writes — the Codex
update skip and its retries, the Claude browser-tools decline, and trust
navigation. Each observes everything received and re-checks the settled frame
before writing, so an automated key cannot reach a trust dialog that arrived
while the previous screen was still rendering. A withheld automated write is
silent: it is not a write failure, emits no `startup_prompt` activity and no
warning, and the prompt stays answerable on a later frame. Cancellation
