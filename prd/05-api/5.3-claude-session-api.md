...
Native `user_message` activity or observed native working evidence permanently
revokes background recovery for a submission captured before its paste and first
Enter. This includes evidence received while that physical Enter is pending and
working evidence followed by an idle repaint. Native working evidence is gated
by initial readiness and the resume replay quiet-frame rule, independently of
optimistic lifecycle running status. Startup spinners and replayed historical
work cannot revoke recovery. Optimistic caller-submitted status
does not revoke recovery. Revocation conservatively withholds further Enter; it
does not prove correlation to a particular prompt, change the first-write promise,
or release composer cleanup ownership.
