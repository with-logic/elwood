...
quiet within one second, or the latest render failed — the write is withheld
...
(`sendInput`, `writeOutput`, `resize`, `settled`, `renderFailed`, and `dispose`); callers should
...
which covers output received while waiting and never rejects; `renderFailed` is
true while the latest output failed to render — and read `terminal.snapshot()`, but visual terminal renderers SHOULD render the raw
