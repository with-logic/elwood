...
The queue wait and picker operation share the supplied `timeoutMs` budget;
termination aborts pending navigation. On failure, Elwood attempts to cancel only
its recognized model dialog and waits up to one additional second for it to clear
before releasing the slot. A submitted `/model` whose dialog has not rendered by
the failure can still open it, and an accepted stage can still paint its follow-up
(Codex's reasoning level, Claude's cache warning), so Elwood holds the slot for
that same bounded second, cancelling a dialog that appears late. An Escape the
operation already wrote is not repeated, and termination stops recovery before
any further write. If it remains, normal queued input stays held while
that dialog is visible; raw `sendKeys` remains available for manual recovery.
A model dialog already visible when the operation acquires the slot (a human's
picker, or transcript text quoting one) is not Elwood's: the operation rejects
with `model_automation_failed` without writing, cancelling, or holding input.
Codex retains its outer process-wide configuration transaction lock.
