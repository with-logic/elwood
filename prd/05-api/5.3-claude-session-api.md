...
The per-session control queue accepts at most **1,024 outstanding operations**
and **8 MiB (8 × 1,024 × 1,024 bytes) of aggregate UTF-8 input text**, including
its active physical operation. This shared limit covers queued messages, prompts,
guidance, commands, exclusive controls, and loop-origin submissions. An admission
that would exceed either limit rejects with `input_queue_full` before attachment,
admission preparation, or terminal input; existing operations keep their order.
Text is measured before paste sanitization. Image clones retain their separate
§5.3 budget. Cancelling a waiting operation or physically settling an active one
releases its reservation; cancelling an active write does not release capacity
before that write settles. Physical loop completion releases capacity before
`fired` observers run, while retaining queue ordering through the following
running notification. Session close rejects waiting operations and releases
their reservations; new calls on a closed session still reject with
`session_not_running`, even if the queue was full. Direct `sendKeys`, terminal
input, and interrupt remain available outside this queue. Capacity is a rejection
boundary, not another unbounded waiting list. A loop whose admission is rejected
follows the existing failed-submission retry/warning policy (§5.9), never `fired`.
...
composer), Elwood may re-send Enter at most twice, over at most four observation
attempts spaced one second apart after the first Enter. Only the completed,
visible, idle composer at the native cursor can establish staged text; matching
transcript text is insufficient. Working, hidden, incomplete, and unknown frames
withhold Enter and consume an observation attempt, not a physical retry. A fresh
positive empty-input observation retires that submission's recovery permanently;
an empty frame retained from before its paste does not. Dialogs withhold recovery
under the same observation bound. This bound does not shorten the initial
submission's dialog wait.

Working indicators alone cannot distinguish accepted work from startup or resume
replay. A later verified idle staged draft may therefore recover, whether or not
initial readiness has fired. Recovery does not infer acceptance from optimistic
status or permanently remember a working indicator. A cached staged frame is not
proof that the native CLI has not accepted the submission.
