...
Acceptance recovery is bounded by the turn itself: it is disarmed the moment the
turn settles, however it settles — a successful boundary, a consumer-facing
failure, a terminal status, or a submission that rejected outright. A settled turn
has nothing left to recover, so no replay may fire after it. Otherwise a turn that
already failed could re-submit its prompt into whatever the session does next,
overlapping an unrelated turn with a prompt whose own turn is over.

Settling also cancels any recovery submission already queued or writing. A queued
replay is removed; an in-flight replay stops before any further prompt or Enter
write, including while a dialog holds input. The turn boundary retains the
serializer slot and captured images until those cancelled writes finish settling
(including cleanup or rejection). Cancellation must not wait for the held dialog
to clear, and no stale replay may submit after it clears.

A turn-acceptance replay also observes the settled native composer after its last
bounded Enter retry. If the prompt remains staged, the replay rejects with
`wait_timeout` and retains draft cleanup ownership. A successor cannot attach
images or write input until that owned draft has been cleared and observed empty.
Writing the final Enter alone does not release ownership or prove acceptance.
...
later queued operation dispatches, because the composer then belongs to it. `sendKeys` remains the explicit raw-input escape hatch and
does not wait.
