...
`cleanupErrorCode` when probe termination failed, and bounded captured stderr —
...
without deleting the owner’s lease. The lease records the owner's process id and a unique
...
a dead owner's surviving group retains exclusion. Registration writes that finish
late retain lease ownership until they settle, so they cannot overwrite a successor.
Aborted probes retry process-group termination,
...
additional one-second bound; unresolved process groups retain update exclusion
as described above. Truncation happens on
