...
| `wait_timeout` | A `waitForStatus`/`waitForActivity` call did not observe its condition before the timeout, or the PTY rejected a staged-draft cleanup write before successor input. The same code rejects queue-backed input admitted from an expired Stop callback/observer context (§6.3). Cleanup failures retain the pending draft and use a bounded message without raw PTY diagnostics. |
