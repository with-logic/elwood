 ...
 * Implements PRD §5.3 and C-API-42: when recording Claude's one-shot initial-ready
 * transition throws, Elwood still releases the control queue directly so queued
 * input is never starved, but persisted status and emitted lifecycle events may be
 * stale. This surfaces that risk as a typed `initial_ready_fallback` warning
 * carrying only a bounded reason — never a raw system message or session content.
 */
