...
import { claudeTextStaged } from "../composer/staged-text.ts";
...
export class ClaudeSessionImpl extends AgentSessionBase implements ClaudeSessionApi {
  ...
  protected stagedPaste(): boolean {
    return claudeTextStaged(this.terminal);
  }
  ...
  login(options: ClaudeLoginOptions): Promise<void> {
    ...
    const terminal = this.automatedTerminal;
