...
export function drainToBudget(
  ...
  for (const cursor of cursors) {
    const hasBacklog = drainCursor(context, cursor, budget, deadline, now);
    // An unread teardown backlog is content-free data loss with its OWN cause
    // ("unread_backlog"), never conflated with an unparseable record: it surfaces as
    // ONE live drop warning under a truthful cause (C-CLAUDE-15, PRD §5.4).
...
// Returns whether bytes remained unread when the budget or the deadline was spent
// (false when fully drained), for backlog accounting.
function drainCursor(
  ...
  while (budget.chunks > 0 && now() < deadline) {
    ...
    if (chunk === undefined) return false; // contained FS failure; nothing to account
    if (chunk.text.length > 0) context.lines.emitLines(cursor.path, chunk.text, cursor);
    if (!chunk.canContinueNow) return false;
  }
  return (context.readFs(cursor.path, () => cursor.remainingBytes()) ?? 0) > 0;
}
