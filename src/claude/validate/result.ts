...
import type { ClaudeHookEvent, ClaudeHookEventName, ClaudeHookResult } from "../hooks/index.ts";
import { isPermissionUpdateArray } from "./permission-update.ts";
import { isClaudeToolInputUpdate } from "./tool-update.ts";
...
// Every field is optional, so `{}` is a valid (no-op) result, as it is for context events.
function isPostToolUseResult(value: Readonly<Record<string, unknown>>): boolean {
  return (
    keysAre(value, ["additionalContext", "updatedToolOutput", "updatedMCPToolOutput"]) &&
    optionalString(value["additionalContext"])
  );
}
...
function isContextResult(value: Readonly<Record<string, unknown>>): boolean {
  return (
    keysAre(value, ["additionalContext", "initialUserMessage", "watchPaths"]) &&
    optionalString(value["additionalContext"]) &&
