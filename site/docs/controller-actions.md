...
The landing-page controller calls `SpriteBank.prepareAnimation(name)` before delivering an explicit gesture or facing request, preparing every sheet of that clip, idle, and rotation. Canceled requests do not deliver input. `SpriteBank.prepare(name)` remains the metadata-and-entry-page loader for on-demand transitions. Decoded resources belong to the four-page cache, retained current/outgoing poses, and the displayed prepared clip, one delivered request awaiting playback, and one pending preparation; these owners may share pages. A prepared hover retains its full clip during playback, while static held poses display one current page. Both use the reduced resting paint cadence. Full source clips, 4K videos, native PNGs and the archive sprite player remain independent of the game’s hold behavior.

## Keyboard

The page exposes all 28 emotes on labeled, clickable QWERTY keys. Each has one `data-code` matching a physical key; no Shift combinations are required. Movement, facing, pause and reset occupy their existing keys. Virtual movement supports pointer presses and focused Space/Enter activation, releasing on pointer cancellation, keyup or blur. On the landing page, explicit gesture and facing requests wait for every required sheet before delivery; automatic transitions retain lazy preparation. The internal workshop retains metadata-and-entry-page preparation. Canceled preparation resolves to `null` and delivers no controller input; asset failures reject and report the failing asset path. Cancellation releases pending candidate and delivered owners, discarding queued explicit input while preserving active playback and outgoing poses. Responsive reflow and reset invalidate preparation before clearing the world, so a late completion cannot restore dropped input.
