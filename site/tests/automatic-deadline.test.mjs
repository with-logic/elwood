...
for (const kind of ["moment", "face"]) {
  for (const elapsed of [7.99, 8, 8.01]) {
    test(`${kind} becoming ready at ${elapsed}s respects the pre-delivery deadline`, async (t) => {
      const { scene, bank, gates, requested } = await setup(t);
      const name = kind === "moment" ? "wave" : "idle-back";
      const gate = Promise.withResolvers();
      gates.set(`${name}/0.webp`, gate);
      const task = { kind, name, direction: "back", sent: false, elapsed: 0, hold: 3 };
      ...
      scene.step(0.01);
      assert.equal(task.sent, elapsed <= 8);
      assert.equal(scene.director.task.kind, elapsed <= 8 ? kind : "rest");
    });
  }
  test(`expired ${kind} does not start asset or geometry work`, async (t) => {
    const { scene, requested } = await setup(t);
    scene.director.task = { kind, name: "wave", direction: "back", sent: false, elapsed: 8 };
    const name = kind === "moment" ? "wave" : "idle-back";
    scene.step(0.01);
    await turn();
...
test("the pre-delivery deadline does not cancel an already delivered held gesture", async (t) => {
  ...
  const task = { kind: "moment", name: "wave", sent: true, elapsed: 9, hold: 3 };
  scene.director.task = task;
