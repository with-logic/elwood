...
`startCodex`, raw `activity`/`warning`/`terminal:exit` events, an explicit
`waitForSettled` helper (from `support.ts`) that watches `status` and
`terminal:exit`, and CLI flags for agent/cwd/prompt/timeout. Reach for this when you
...
kill the example process tree on Ctrl-C. The examples import from `../src/index.ts`
so they run from a checkout without a build; in your own project import the same
symbols from `@with-logic/elwood`.
