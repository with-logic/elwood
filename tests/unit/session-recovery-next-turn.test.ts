...
for (const agent of ["claude", "codex"] as const)
  for (const repaint of [
    ...
    test.each([
      "queued",
      "ready-listener",
    ] as const)(`C-API-31 ${agent} %s successor after Stop with ${repaint} repaint`, async (arrival) => {
      ...
      const paint = (working = false) =>
        pty.emitData(
          `\u001b[2J\u001b[H${tty(draft)}\u001b]0;${working ? "⠋ Working" : "Ready"}\u0007`,
        );
      ...
      vi.spyOn(pty, "write").mockImplementation((value) => {
        ...
        if (String(value).startsWith("\u001b[200~")) {
          if (++pastes === 2) {
            if (repaint === "brief-quiet") setTimeout(() => paint(), 25);
            setTimeout(() => paint(true), 50);
            setTimeout(() => paint(), 100);
            if (repaint.startsWith("early-")) {
              setTimeout(() => {
                const idle = agent === "claude" ? claudeComposer : codexSmallComposer;
                const submitted =
                  agent === "claude" ? "❯ [Pasted text #1 +15 lines]" : "› successor";
                const history = repaint === "early-history" ? `${submitted}\n` : "";
                pty.emitData(
                  `\u001b[2J\u001b[H${tty(`${history}${idle}`)}\u001b]0;⠋ Working\u0007`,
                );
              }, 200);
            }
            if (repaint === "fresh-working") {
              for (let at = 150; at <= 350; at += 50) setTimeout(() => paint(), at);
              setTimeout(() => paint(true), 400);
            }
          } else paint();
        }
      });
      try {
        ...
        expect(pty.writes.filter((value) => value === "\r")).toHaveLength(
          repaint === "fresh-working" || repaint.startsWith("early-") ? 2 : 3,
        );
      } finally {
        vi.useRealTimers();
