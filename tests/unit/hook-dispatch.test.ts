/**
 * Hook dispatch timeout-classification and fail-open coverage.
 * Covers PRD §6.3 (timeout vs handler_error, timeoutMs only on genuine timeout).
 */

import { describe, expect, test } from "vitest";
import { isBlock, requestHook } from "../../src/claude/hook-dispatch.ts";
import { isCodexBlock, requestCodexHook } from "../../src/codex/hook-dispatch.ts";
import type { CodexEventMap } from "../../src/codex/session-types.ts";
import type { HookErrorEvent } from "../../src/core/types.ts";
...
function collectErrors(emitter: TypedEmitter): HookErrorEvent[] {
...
describe("Claude hook dispatch timeout classification", () => {
  test("C-HOOK-16 a genuine timeout is categorized as timeout with timeoutMs", async () => {
  ...
  test('C-HOOK-16 a handler that throws Error("timeout") is a handler_error without timeoutMs', async () => {
    const emitter = new TypedEmitter();
    const errors = collectErrors(emitter);
  ...
  test("C-HOOK-16 a non-Error throw fails open with a generic handler_error message", async () => {
    const emitter = new TypedEmitter();
    const errors = collectErrors(emitter);
  ...
  test("C-HOOK-16 an invalid response fails open with invalid_response", async () => {
    const emitter = new TypedEmitter();
    const errors = collectErrors(emitter);
  ...
  test("C-HOOK-16 a valid response passes through without an error", async () => {
    const emitter = new TypedEmitter();
    const errors = collectErrors(emitter);
  ...
  test("C-HOOK-16 no registered listener fails open without an error", async () => {
    const emitter = new TypedEmitter();
    const errors = collectErrors(emitter);
    const outcome = await requestHook(emitter, claudeStop, 1000, "sess-1");
...
describe("Codex hook dispatch timeout classification", () => {
  test("C-HOOK-16 a genuine Codex timeout is categorized as timeout with timeoutMs", async () => {
  ...
  test('C-HOOK-16 a Codex handler throwing Error("timeout") is a handler_error', async () => {
    const emitter = codexEmitter();
    const errors = collectCodexErrors(emitter);
    emitter.on("hook:Stop", () => {
      throw new Error("timeout");
  ...
  test("C-HOOK-16 a Codex non-Error rejection fails open generically", async () => {
    const emitter = codexEmitter();
    const errors = collectCodexErrors(emitter);
  ...
  test("C-HOOK-16 a Codex invalid response fails open with invalid_response", async () => {
    const emitter = codexEmitter();
    const errors = collectCodexErrors(emitter);
    emitter.on("hook:Stop", () => ({ decision: "nope" }));
    const outcome = await requestCodexHook(emitter, codexStop, 1000, "sess-2");
  ...
  test("C-HOOK-16 a valid Codex response passes through", async () => {
    const emitter = codexEmitter();
    emitter.on("hook:Stop", () => ({ decision: "block", reason: "stay" }));
    const outcome = await requestCodexHook(emitter, codexStop, 1000, "sess-2");
    expect(outcome.failedOpen).toBe(false);
  ...
  test("C-HOOK-16 no Codex listener fails open without an error", async () => {
    const emitter = codexEmitter();
    const errors = collectCodexErrors(emitter);
    const outcome = await requestCodexHook(emitter, codexStop, 1000, "sess-2");
    expect(outcome).toEqual({ result: undefined, failedOpen: false });
...
describe("hook block detection", () => {
  test("C-HOOK-16 isBlock recognizes only a block decision", () => {
  ...
  test("C-HOOK-16 isCodexBlock recognizes block decisions and continue:false", () => {
