/** Byte-based terminal backpressure regressions (PRD §4.1, C-PERF-05). */
...
test("C-PERF-05 staged and submitted UTF-8 bytes share the pause/resume thresholds", async () => {
...
test("C-PERF-05 disposal resumes paused PTYs and ignores later completions/input", async () => {
  const { output, flow, callbacks } = controlledOutput();
...
test("C-PERF-05 adapters without flow control still render a burst", async () => {
  let rendered = 0;
