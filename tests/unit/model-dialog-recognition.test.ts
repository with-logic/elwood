...
test.each([
  ...
  expect(spec.activeDialog(forged, opened)).toBeUndefined();
});
...
test.each([
  ...
  expect(spec.activeDialog(captured, opened)).toBe(stage);
});
...
test("C-API-24 a region with two cursor rows is not one native picker", () => {
  ...
  expect(claudeModelPicker.activeDialog(claudePicker, opened)).toBe("picker");
  expect(claudeModelPicker.activeDialog(twoCursors, opened)).toBeUndefined();
});
