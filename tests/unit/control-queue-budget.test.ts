...
test("C-API-58 rejects a single oversized input without retaining capacity", async () => {
  ...
  const oversized = queue
    .send("x".repeat(8 * 1_024 * 1_024 + 1), "message")
    .catch((error: unknown) => {
      failure = error;
    });
  try {
    ...
    expect(failure).toHaveProperty("message", expect.stringContaining("reduce the input size"));
...
test("C-API-58 cancelling queued work releases its UTF-8 and operation reservation", async () => {
  ...
  const input = "x".repeat(8 * 1_024 * 1_024);
  const first = observe(
...
test.each([
  ...
  const first = observe(
    queue.send("x".repeat(8 * 1_024 * 1_024), "prompt", undefined, {
...
test("C-API-58 full queue close preserves stopped precedence and rejects all waiting input", async () => {
  ...
  const waiting = Array.from({ length: 1_024 }, () => observe(queue.send("", "message")));
  queue.close();
  queue.close();
  expect((await Promise.all(waiting)).every((error) => error === stopped)).toBe(true);
  expect(await observe(queue.send("x".repeat(8 * 1_024 * 1_024 + 1), "prompt"))).toBe(stopped);
});
...
test("C-API-58 exclusive controls share count capacity and queued cancellation frees one slot", async () => {
  ...
  const pending = Array.from({ length: 1_024 - 1 }, () => observe(queue.send("", "message")));
