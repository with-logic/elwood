...
describe("Codex bounded cursor-io", () => {
  test("C-API-12 fileSize returns 0 for a missing file and rethrows other errors", () => {
  ...
  test("C-API-12 readRange decodes only to a complete UTF-8 boundary via the seam", () => {
...
describe("Codex bounded cursor", () => {
  test("C-API-12 readChunk caps at 256 KiB and streams the remainder", () => {
  ...
  test("C-API-12 readChunk restarts at 0 on truncation and reports no growth", () => {
    ...
    // branch), returning more:false without a read.
  ...
  test("C-API-12 takeLines discards an over-length un-terminated record", () => {
  ...
  test("C-API-12 takeLines can END one discard and START the next in one chunk", () => {
    const cursor = new CodexTranscriptCursor(join(tempDirForUnit(), "e.jsonl"));
  ...
  test("C-API-12 drainPending flushes then clears, remainingBytes reflects the tail", () => {
  ...
  test("C-API-12 remainingBytes is 0 when the file shrank below the cursor", () => {
