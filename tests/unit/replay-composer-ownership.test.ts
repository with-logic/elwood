...
test.each([
  ...
  try {
    queue.markReady();
    const replay = queue.send("replay", "message", undefined, replayControl(abort.signal));
...
test("C-API-56 successful replay releases its draft ownership after nudges settle", async () => {
  ...
  const queue = new ControlQueue(
    replayInput(terminal, { snapshot: () => "replay", staged: () => staged }),
    () => new Error("session_not_running"),
    () => undefined,
    () => false,
    undefined,
  ...
  try {
    ...
    const replay = queue.send(
      ...
      "message",
      undefined,
      replayControl(new AbortController().signal),
    );
