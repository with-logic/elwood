/** Terminal render throughput and backpressure regressions (PRD §4.1, C-PERF-05). */
...
test("C-PERF-05 submits consecutive writes to xterm without serial timer barriers", async () => {
...
test("C-PERF-05 adjacent PTY chunks share a frame while later batches remain ordered", async () => {
...
test("C-PERF-05 disposal settles queued writes even when xterm abandons callbacks", async () => {
  const terminal = createHeadlessTerminal({ cols: 20, rows: 3 }, () => undefined);
  vi.spyOn(terminal.xterm, "write").mockImplementation(() => undefined);
  const completed = vi.fn();
  void terminal.writeOutput("pending").then(completed);
