/** Native submission ends subsequent background Enter attempts (C-API-31). */
...
for (const agent of ["claude", "codex"] as const) {
  test(`C-API-31 ${agent} native submission prevents later recovery Enter`, {
      ...
      return {
        ...
        write(data) {
          if (armed && String(data) === "\r") {
            ...
            // Pre-hook retries are outside this hook-only guarantee. Count and
            // intercept every retry, including those that must fail the assertion.
            if (attempted > 1) {
    ...
    try {
      ...
      for (const phase of ["cold", "resume"] as const) {
        ...
        try {
          ...
          await waitFor(
            () =>
              observed.activities.some((event) => event.kind === "user_message") ? true : undefined,
