/**
 * Effective-setting inspection and validation tests (PRD §12A.4, C-CLI-19).
 */
...
describe("effective config", () => {
  test("C-CLI-19 can ignore defaults and report debug provenance", async () => {
    const h = harness();
    ...
    expect(JSON.parse(h.writer.value)).toMatchObject({
      ...
      config: { loaded: false },
      settings: {
        agent: { value: "codex", source: "built-in" },
  ...
  test("C-CLI-19 reports headed output mode and provenance", async () => {
    const h = harness();
    await writeEffectiveConfig(["--head"], h.context);
  ...
  test("C-CLI-19 reports unset Claude tool policy on a new session", async () => {
    const h = harness();
    await writeEffectiveConfig(["--agent", "claude"], h.context);
    expect(JSON.parse(h.writer.value).settings).toMatchObject({
  ...
  ])("rejects non-setting input %#", async (args, message) => {
    const h = harness();
